...
// background: linear-gradient(83deg, rgba(17,157,164,1) 51%, rgba(234,240,206,1) 92%);
    ...
    <img src={phots} alt="" className="absolute -z-10 w-[100%]  text-center   opacity-[0.35] "/>
    ...
    {/* <img src={map3} className="absolute -z-10 w-[100%]  text-center   opacity-[0.5] mix-blend-color-dodge" alt="" /> */}
      <div className="flex flex-col justify-center font-poppins items-center h-[90vh]  w-screen">
        <div className="text-3xl font-bold text-primary1">We Bring</div>
        <div className="flex flex-col sm:flex-row">
          {swag.map((item, index) => (
            <div key={index}>
              <h1 className="text-6xl ">
                <span className="text-9xl font-bold text-primary1">{item.title}</span>
        ...
        <div>
          <h1 className=" text-3xl pt-4 font-bold text-primary1">to the </h1>
        </div>
        <div>
          <h1 className="text-8xl font-bold text-gradient-to-r from-primary1 to-newsletter text-primary1" >Businesses</h1>
        </div>
      </div>
    </div>
  );
