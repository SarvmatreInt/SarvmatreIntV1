	...
	return (
		<div className=" bg-gradient-to-r bg-primary1  absolute w-full left-0">
			<div className="max-w-7xl my-0 mx-auto px-[1rem] pt-[3.47rem] pb-[1rem] lg:flex gap-5 relative">
				{/* <img src={footerdesign} alt="" className="absolute right-[5%] max-w-[200px]  z-[-1] top-0"/> */}
				<div className="flex flex-col gap-2 max-w-xs">
					<img src={logo} alt="" className="max-w-[150px]"/>
					<p className="text-sm text-white">
						Sarvmatre International Private Limited Jain Niwas, Tali Morh, Near Doordarshan Kendra, Old Janipur, Jammu, J&K, India - 180007
					</p>
					<a href="mailto:corp@sarvmatre.com" className="text-sm text-white w-fit">corp@sarvmatre.com</a>
				...
				<div className="flex  flex-wrap xsm:grid xsm:grid-cols-2 sm:grid-cols-4 mt-8 gap-16 w-full z-40">
					{
						footer_data.map((data, index) => {
							return (
								<div className="flex  flex-col justify-start text-start text-white max-w-full" key={index}>
									<h3 className="font-bold text-2xl mb-1">{data.heading}</h3>
									<ul className="flex flex-col gap-2">
										{
											data.tags.map((tag, index) => {
												return(
													<li key={index}><Link to={`${import.meta.env.VITE_BASE_URL}/${tag.link}`}>{tag.heading}</Link></li>
												)
											})
										}
									</ul>
			...
			<p className="py-10 mx-2 text-white text-xs text-center select-none">
				Copyright © 2023 SIPL | All Rights Reserved | <a href="/" className="hover:underline">Terms and Conditions</a> | <a href="/" className="hover:underline">Privacy Policy</a>
			</p>
		</div>
	);
};
